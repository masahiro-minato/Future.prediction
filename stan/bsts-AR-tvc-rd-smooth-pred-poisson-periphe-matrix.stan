...
data {
  ...
  // int y[N,T];                      // 観測値 旧表記
  array[N] vector<lower=0>[T] Num;    // 稼働台数
  // array[N,T] real<lower=0> Num;       // 稼働台数
  array[N] real<lower=0> N_max;       // 稼働台数を正規化時の除算数
...
generated quantities {
  ...
  array[N] vector[T+pred_term] Num_pred;                          // 稼働台数
  ...
  for (n in 1:N) {
    for (i in 1:pred_term) {
      ...
      // 想定稼働台数
      Num_pred[n,T+i] = Num_pred[n,T+i-1] + Num_gain[n];
      // 時変係数成分 tvc
      tvc_pred[n,(T+i)] = b_ope_pred[n,(T+i)] * Num_pred[n,T+i];
      // 状態推定値 λ
  ...
  for (n in 1:N) {
    ...
    mu_tvc_1_exp[n,] = exp(mu_pred[n,] + b_ope_pred[n,] / N_max[n]);
    mu_tvc_1[n,] = mu_pred[n,] + b_ope_pred[n,] / N_max[n];
    tvc_1_exp[n,] = exp(b_ope_pred[n,] / N_max[n]);
    tvc_1[n,] = b_ope_pred[n,] / N_max[n];
    // 観測値の予測値算出
