...
transformed parameters {
  ...
  // ランダム効果成分 r
  r = s_r * r_raw[1:T];   // 再パラメータ化
  // 水準成分 μ, 稼働台数時変係数 b_ope
  // 1時点目
  ...
  for (i in 2:T) {
    ...
    mu[i] = Intercept + b_ar * mu[i-1] + s_w * mu_err[i];  // 再パラメータ化
    b_ope[i] = b_ope[i-1] + s_t * b_ope_err[i];            // 再パラメータ化
  }
  // 周期成分 γ
  gamma[1:6] = gamma_err[1:6];
  for(i in 1:T-6) {
    sumgamma[i] = -sum(gamma[(i):(i+5)]);
    gamma[i+6] = sumgamma[i] + s_s * gamma_err[i];         // 再パラメータ化
  }
  // 状態推定値 λ
  for(i in 1:T) {
    tvc[i] = b_ope[i] * Num[i];
...
generated quantities {
  ...
  // ランダム効果成分 r, 水準成分 μ, 稼働台数時変係数 b_ope
  for (i in 1:pred_term) {
    r_pred[T+i] = normal_rng(0, s_r);
    mu_pred[T+i] = normal_rng(Intercept + b_ar * mu_pred[T+i-1], s_w);
    b_ope_pred[T+i] = normal_rng(b_ope_pred[T+i-1], s_t);
  }
  // 周期成分 γ
  for(i in 1:pred_term) {
    sumgamma_pred[T-6+i] = -sum(gamma_pred[(T-6+i):(T-6+i+5)]);
    gamma_pred[T+i] = normal_rng(sumgamma_pred[T-6+i], s_s);
  }
  // 状態推定値 λ
  for(i in 1:pred_term) {
  ...
  // パラメータ設定
  vector[T+pred_term] lambda_exp;      // 状態推定値(EXP)
  vector[T+pred_term] mu_gamma_exp;    // ランダム効果を除いた状態推定値
  vector[T+pred_term] mu_r_exp;        // 周期成分を除いた状態推定値
  vector[T+pred_term] mu_tvc_exp;      // 水準成分+時変係数成分
  vector[T+pred_term] mu_exp;          // 周期成分・ランダム効果を除いた状態推定値
  vector[T+pred_term] gamma_exp;       // 周期成分(EXP)
  vector[T+pred_term-31] delta_exp;    // ドリフト成分(EXP)
  vector[T+pred_term] r_exp;           // ランダム成分(EXP)
  vector[T+pred_term] tvc_exp;         // 時変係数*稼働台数成分
  vector[T+pred_term] mu_tvc_1_exp;    // 稼働台数を1台に設定(EXP)
  vector[T+pred_term] mu_tvc_1;        // 稼働台数を1台に設定
  vector[T+pred_term] tvc_1_exp;       // 稼働台数時変係数(EXP)
  vector[T+pred_term] tvc_1;           // 稼働台数時変係数,EM率相当
  array[T+pred_term] int y_pred;       // 観測値の予測値
