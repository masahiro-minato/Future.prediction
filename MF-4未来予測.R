...
# ファイル読込
MF4_maintenance <- read_tsv("./tsv_data/Metis_MF4_2305.tsv")　    # 保守データ
MF4_machine <- read_tsv("./tsv_data/MF4_MIF_2305.tsv")            # 機器データ
...
data_list <- list(
  ...
  grainsize = 250,
  pred_term = pred_term,
  Num_gain = c(70,20,10,8)/unlist(c(max_data.MF4)) # 日次での稼働台数増加予測数 c(100,20,10,8)
)
# コンパイル
